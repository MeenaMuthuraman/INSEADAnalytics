...
**Your Answers here:**
<br>
[Downloading foreign exchange data from Oanda] (http://www.quantmod.com/documentation/getFX.html)
...
**Your Answers here:**
<br>
1. The mytickers variable in the dataSet1 file has SPY and AAPL (the ticker symbol for Apple) as inputs in this line mytickers = c("SPY", "AAPL"). Hence, when the code retrieves data it retrieves that of Apple too.
<br>
2. 
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
...
pnl_plot(AAPL)
```
  The cumulative daily returns of Apple is `r round(100*sum(StockReturns[,2]),1)` %.
<br>
   The average daily returns of Apple is `r round(100*mean(StockReturns[,2]),3)`%.
<br>
   The standard deviation of the daily returns for the Apple is `r round(100*sd(StockReturns[,2]),3)`%.
<br>
3. To get information on Yahoo!, the line mytickers = c("SPY", "AAPL") in dataSet1 should be changed to     mytickers = c("SPY", "AAPL", "YHOO"), including the ticker for Yahoo.
<br>
   The cumulative daily returns of Yahoo is `r round(100*sum(StockReturns[,3]),1)` %.
<br>
  The average daily returns of Yahoo is `r round(100*mean(StockReturns[,3]),3)`%.`
<br>
  The standard deviation of the daily returns for the Yahoo is `r round(100*sd(StockReturns[,3]),3)`%.
<br>
<hr>
<hr>
...
**Your Answers here:**
<br>
1. The nrow and ncolumns commands must be used.`r transposedData = t(StockReturns)`
<br>
The number of rows is `r nrow(transposedData)`.
<br>
The number of columns is `r ncol(transposedData)`.
<br>
2.
...
**Your Answers here:**
<br>
1. The start date in the dataSet1 file should be 2001-01-01, startDate = "2001-01-01". The present exercise set already has this date set as the start date.
<br>
2. I can add more ticker symbols to mytickers = c("SPY", "AAPL", "YHOO") such as mytickers = c("SPY", "AAPL", "YHOO", "GOOG", "MSFT"). Then I can apply the code in part 2 above to plot the portfolio.
...
```
<br>
The sum command gives me 20 as the answer, which is the number of data points that I read and wrote for the stock returns of SPY and AAPL. However the output is a lump of data with dates and stock returns of SPY and AAPL all together that is meaningless and the code above should have sep";" changed to sep"," so that the data is presented with clarity under 3 headers - date, SPY and AAPL.
<br>
2. You will get an error as the variables myData and StockReturns dont match up in terms of dimensions. myData has date, SPY and AAPL returns all lumped together with 20 observations, while StockReturns has date as a header and returns of SPY, AAPL and YHOO for 40 observations.
...
<br>
```{r echo=TRUE, comment=NA, warning=FALSE, message=FALSE,results='asis',fig.align='center', fig.height=4,fig.width= 6, fig=TRUE}
myTempData <- read.csv(file = "annualglobaltemperatures.csv", header = TRUE, sep=",")
class(myTempData)
dim(myTempData)
nrow(myTempData)
ncol(myTempData)
...
```
<br>
I have loaded data on average global mean temperature anomalies in degrees Celsius relative to a base period. Fpr GISTEMP, the base period is the average between 1951-1980 and for GCAG, the base period is the 20th century average.For this data it has 272 and 3 as dimensions with 272 rows and 3 columns.
<br>
<br>
